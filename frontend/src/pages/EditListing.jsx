...
const EditListing = () => {
    ...
    const fetchListing = async () => {
      try {
        ...
        setFormData({
          ...
          price: listing.price || '',
          priceType: listing.priceType || 'total',
          ...
          furnished: listing.furnished || 'unfurnished',
        ...
        setPreview(listing.images?.[0]?.url || listing.image?.url || listing.image);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`/listings/${id}`, submitData, {
    ...
    <div className="container mx-auto px-4 md:px-8 max-w-4xl pt-28 pb-20">
      ...
      >
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-secondary mb-2">Edit Property</h1>
            <p className="text-muted">Update details for your property listing.</p>
          </div>
          <Button type="button" label="Cancel" variant="outline" onClick={() => navigate(-1)} />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <motion.div variants={sectionVariants} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <div>
              <label className="text-sm font-semibold text-secondary mb-1.5 block">Property Images (Select New Cover Image to Replace)</label>
              <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors relative">
                ...
                {preview ? (
                  <div className="w-full relative h-64 rounded-xl overflow-hidden shadow-sm group">
                    <img src={preview} alt="Preview" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="text-white font-semibold flex items-center gap-2"><UploadCloud className="w-5 h-5" /> Change Image</span>
                    </div>
                  </div>
                ) : (
                  <>
                    <UploadCloud className="w-12 h-12 text-muted mb-3" />
                    <p className="font-semibold text-secondary">Click to upload a new image</p>
                    <p className="text-sm text-muted mt-1">Leave empty to keep current image</p>
                  </>
                )}
              </div>
          ...
          <motion.div variants={sectionVariants} className="pt-4 flex justify-end">
            <div className="w-full md:w-1/3">
              <Button type="submit" label={submitting ? 'Saving...' : 'Save Changes'} fullWidth loading={submitting} />
            </div>
          </motion.div>
